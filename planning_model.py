...
def dynamic_constraint(lon1, lat1, yaw1, v1, acc1, current_v, current_a, T):
    max_acc = 2
    max_speed = 15
    min_turning_radius = 8
    
    # speed and acceleration constraint
    acc1 = np.clip(acc1, -max_acc, max_acc)
    v1 = np.clip(v1, current_v - T * max_acc, current_v + T * max_acc)
    v1 = np.clip(v1, 0, max_speed)

    # displacement constraint
    lon1 = np.clip(lon1, 0, max_speed * T) 
    lat1 = np.clip(lat1, - max_speed * T / 2, + max_speed * T / 2)
    
    # turning constraint
    if lon1 <= min_turning_radius: 
        lat1 = np.clip(lat1, -lon1, lon1)

    return lon1, lat1, yaw1, v1, acc1
...
def motion_skill_model(lon1, lat1, yaw1, v1, acc1, current_v, current_a, horizon=60):

    # lat1, yaw1, current_v, current_a, v1 = dynamic_constraint(lat1, yaw1, current_v, current_a, v1, horizon)
    lon1, lat1, yaw1, v1, acc1 = dynamic_constraint(lon1, lat1, yaw1, v1, acc1, current_v, current_a, horizon/10)
    ...
    SpeedProfile = SpeedParam(v0=current_v, acc0=current_a, v1=v1, T=horizon/10)
    ...
    # in case the accumulated distance from speed profile exceeds the path
    if dist_lst[-1] > Path.path_length[-1]: 
        scale_ratio = (Path.path_length[-1]-0.1) / dist_lst[-1]
        scaled_dist_lst = [one_dist * scale_ratio for one_dist in dist_lst]
        dist_lst = scaled_dist_lst
...
# motion_skill_model(lon1, lat1, yaw1, v1, acc1, current_v, current_a, horizon)
